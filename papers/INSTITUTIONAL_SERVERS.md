...
**RESOLVED for CS 218 (2026-08-16, owner-supplied evidence).** The repo owner obtained screenshots of the CS 218 WebCampus (Canvas) pages — the non-public channel the searches above predicted. Local copies: `papers/literature/canvas-cs218/*.png` (gitignored; the owner should record which semester's Canvas shell they came from before citing them). What the pages show **[verified from screenshots]**:

- A page titled **"Ubuntu Development Environment"** requires students to build their own environment and ranks the options: VM software "(Intel x86 only…)" with VirtualBox "the most common"; WSL, which "has been used but is not supported"; or native/dual-boot, "the most complicated, but will likely provide the best performance."
- The course distributes a pre-created **"UNLV CS Ubuntu 24.04 LTS Image"** (`.ova` for VirtualBox import) with g++, ddd, emacs, and VS Code preinstalled — default credentials **student/password**. Note the version: Ubuntu 24.04 LTS is *current* (standard support to 2029), so the blanket claim that the course VM "hasn't been updated" is **not supported for the Intel image** as of this Canvas page.
- A second page, **"Experimental UTM Prebuilt Image for Apple Silicon M1/M2 processors,"** opens: "Apple Macs with the M1 or M2 processors cannot natively run x86_64 assembly." The *primary* guidance for those students is physical: "use the computers in the OIT lab, TBE-A311… Use a USB drive to save your programs." The alternative is an instructor-built UTM image with "no guarantee that the UTM and the provided image will work on your Mac, but you are free to try."
- That Apple Silicon image is where the staleness and fragility live: it is **Linux Mint, not Ubuntu** ("students are responsible to verify that it works correctly in CodeGrade, which uses Ubuntu" — a distro mismatch with the autograder); the UTM panel in the third screenshot names it **"Fall23-studentM1"** (a Fall 2023 build), architecture **x86_64** — i.e., full-system QEMU emulation of x86-64 on ARM — with the page warning that VS Code "may not be usable on your specific machine" because emulation is so slow, recommending nano/vim/geany instead; plus a manual `xrandr` workaround for window resizing and the same student/password defaults.
- The owner reports the download links on these pages could not be reached — consistent with Canvas-internal (or dead) file hosting; there is no public mirror, version history, or changelog for either image.

This evidence also completes a three-corner design space the paper can now populate with same-course exhibits: (1) UNLV's official Apple Silicon route is **full-system emulation** (UTM/QEMU x86_64) — the Cadenas corner, which preserves ddd/gdb inside the guest but is slow enough that the instructor warns VS Code may be unusable; (2) the Docker x86 image on Apple Silicon uses **user-mode translation** — fast, but with the documented gdb/ptrace loss; (3) **native amd64** gets both. No option on ARM hosts gets both speed and the debugger, and UNLV's own materials implicitly concede this by making the physical lab the primary recommendation.
...
| A VM image was officially recommended/distributed for CS 218 | **Confirmed** (owner-supplied Canvas screenshots): "UNLV CS Ubuntu 24.04 LTS Image" (.ova, VirtualBox, Intel only) plus an experimental Fall-2023 Linux Mint UTM image for Apple Silicon; distribution is Canvas-internal with no public mirror or version trail, and the owner reports the download links unreachable |
| The CS 218 Intel VM image is outdated | **Contradicted** — it is Ubuntu 24.04 LTS (current); the staleness case attaches to the Apple Silicon UTM image (Fall 2023 build, Linux Mint/autograder distro mismatch, "experimental… no guarantee") and to the distribution channel, not the Intel image |
| A VM image was officially recommended/distributed for CS 370 | **Not confirmed** — nothing public; 2017 syllabus prescribed cardiac instead |
...
5. **Frame the VM claim carefully (CS 218 / CS 370) — now largely resolved by the Canvas screenshots (§2.4).** The honest, well-evidenced version has three prongs. (a) *Distribution*: the CS 218 images exist only inside Canvas, with no public mirror, dated tags, changelog, or working links the owner could find — a direct contrast with the Docker Hub images' public, auditable, dated tags. (b) *Currency*: do **not** claim the Intel image is stale — it is Ubuntu 24.04 LTS, current until 2029; the evidenced staleness is the **Apple Silicon path**: a Fall 2023 experimental Linux Mint image, distro-mismatched with the CodeGrade autograder, offered with "no guarantee," behind a primary recommendation to physically visit the TBE-A311 lab with a USB drive. (c) *Design*: the same pages hand the paper its sharpest architecture argument — UNLV's own official M1/M2 route is full-system QEMU emulation slow enough that the instructor warns VS Code may be unusable, i.e., the Cadenas trade-off (keep the debugger, pay in speed) exhibited inside the very course the Docker image serves, which takes the opposite corner (speed via user-mode translation, documented gdb/ptrace loss). Also citable: both distributed images ship default credentials (student/password), a security-posture contrast with the containers' localhost-only design. **Remaining TODO (owner):** record which semester's Canvas shell the screenshots came from, and note that CS 370's post-cardiac environment is still unevidenced.
